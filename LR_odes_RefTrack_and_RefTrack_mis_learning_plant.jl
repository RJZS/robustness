...
function LR_observer_RefTrack!(du,u,p,t)
    ...
    du[10] = -V2  -element(V2,afn,dfn) -element(Vs2,asp,dsp) +
        ...
        -element(Vsh2,θ̂-θ̂2,dsn) + kappa * (V - V2)  # Control current
    ...
    ϕ̂2 = -element(Vsh2,1,dsn)

    du[13] = ϕ̂2*θ̂2 - V2  -element(V2,afn,dfn) -element(Vsh2,asp,dsp) +
            -element(Vush2,ausp,dusp) + Iapp + 
            -element(Vsh2,θ̂-θ̂2,dsn) + kappa * (V - V2) + # Control current
            γ*(1+sum(P2.*Ψ2.^2))*(V2-Vh2)

    du[14] = (1/tau_s) * (V2 - Vsh2)
    du[15] = (1/tau_us) * (V2 - Vush2)
    
    du[16]= γ*P2.*Ψ2*(V2-Vh2); # dθ̂ 
    du[18] = -γ*Ψ2 + ϕ̂2;  # dΨ
    du[17] = α*P2 - γ*P2.^2 .* Ψ2.^2;
end
...
function LR_observer_RefTrack_mis!(du,u,p,t)
    ...
    du[10] = -V2  -element(V2,afn,dfn) -element(Vs2,asp,dsp) +
        -element(Vs2,asn2,dsn) -element(Vus2,ausp,dusp) + Iapp +
        -element(Vs2,θ̂-θ̂2,dsn) + kappa * (V - V2) # Control current
    du[11] = (1/tau_s)  * (V2 - Vs2)
    du[12] = (1/tau_us) * (V2 - Vus2)
    ...
    ϕ̂2 = -element(Vsh2,1,delta_ests2[3])

    du[13] = ϕ̂2*θ̂2 - V2  -element(V2,afn,delta_ests2[1]) -element(Vsh2,asp,delta_ests2[2]) +
            -element(Vush2,ausp,delta_ests2[4]) + Iapp + 
            -element(Vsh2,θ̂-θ̂2,delta_ests2[3]) + kappa * (V - V2) + # Control current
            γ*(1+sum(P2.*Ψ2.^2))*(V2-Vh2)

    du[14] = (1/tau_ests2[1]) * (V2 - Vsh2)
    du[15] = (1/tau_ests2[2]) * (V2 - Vush2)
    
    du[16]= t > 32000 ? 0 : γ*P2.*Ψ2*(V2-Vh2); # dθ̂ 
    du[18] = -γ*Ψ2 + ϕ̂2;  # dΨ
    du[17] = α*P2 - α*P2.^2 .* Ψ2.^2;
end