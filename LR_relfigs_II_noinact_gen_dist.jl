...
max_error = 0.00001 # 0.1 # 0.08 # 0.1 gives a mismatch of up to +/- 5%
...
for idx in 1:num_trials
    ...
    # P₀ = ones(10);
    P₀ = [1 1 0 0 1 1 1]' # Matrix(I, 5, 5);
    Ψ₀ = zeros(10);
    # u0 = [x0 xh0 θ̂₀' P₀' Ψ₀']; # For diag observer.
    # u0 = [x0 xh0 θ̂₀' P₀[:]' P₀[:]' Ψ₀']; # For fully dist observer
    u0 = [x0 xh0 θ̂₀' P₀' P₀' Ψ₀'];

    println("Learning...")
    p=(afn,asp,asn,ausp,afn2,asp2,asn2,ausp2,tau_s,tau_us,Iappo,Iappo2,
        asyn21,asyn12,delta_ests_true,delta_ests_true,beta,α,γ,[tau_s, tau_us,
        tau_s, tau_us]);
    probObs = ODEProblem(LR_observer_II_noinact!,u0,tspan,p) # Simulation without noise (ODE)
...
save("sec4_LR_II_noinact_tmp.jld","noise",noise,
