...
class dyns:
    ...
    def mNa_inf(self,V):
        return boltz(V,25*self.mis[0][0],-5*self.mis[0][1])
    #taumNa(V::Float64) = tauX(V,1.32,1.26,120.,-25.)
 
    def tau_mNa(self,V):
        return tauX(V,0.75*self.mis_t[0][0],0.5*self.mis_t[0][1],100.*self.mis_t[0][2],-20.*self.mis_t[0][3])
    #hNainf(V::Float64) = boltz(V,48.9,5.18)

    def hNa_inf(self,V):
        return boltz(V,40*self.mis[1][0],10*self.mis[1][1])
    #tauhNa(V::Float64) = (0.67/(1+exp((V+62.9)/-10.0)))*(1.5 + 1/(1+exp((V+34.9)/3.6)))

    def tau_hNa (self,V):
        return tauX(V,4*self.mis_t[1][0],3.5*self.mis_t[1][1],50.*self.mis_t[1][2],-20.*self.mis_t[1][3])
    #mCaTinf(V::Float64) = boltz(V,27.1,-7.2)
    
    def mt_inf (self,V):
        return boltz(V,60*self.mis[2][0],-5*self.mis[2][1])
    #taumCaT(V::Float64,taumCa::Float64) = taumCa*tauX(V,21.7,21.3,68.1,-20.5)
    
    def tau_mt (self,V):
        return tauX(V,6*self.mis_t[2][0],5.5*self.mis_t[2][1],30*self.mis_t[2][2],-20*self.mis_t[2][3])
        # taumCa*
    #hCaTinf(V::Float64) = boltz(V,32.1,5.5)
    
    def ht_inf(self,V):
        return boltz(V,85*self.mis[3][0],10*self.mis[3][1])
    #tauhCaT(V::Float64) = tauX(V,105.,89.8,55.,-16.9)
    
    def tau_ht(self,V):
        return 100*tauX(V,6.*self.mis_t[3][0],5.5*self.mis_t[3][1],30.*self.mis_t[3][2],-20*self.mis_t[3][3])
    ...
    def tau_mA (self,V):
        mis_t=self.mis_t # Why this line?
    ...
    def mKd_inf (self,V):
        return boltz(V,15*self.mis[9][0],-10*self.mis[9][1])
    #taumKd(V::Float64) = tauX(V,7.2,6.4,28.3,-19.2)
    
    def tau_mKd (self,V):
        return tauX(V,5*self.mis_t[8][0],4.5*self.mis_t[8][1],30*self.mis_t[8][2],-20*self.mis_t[8][3])
    #mHinf(V::Float64) = boltz(V,70.,6.)
    
    def mH_inf (self,V):
        return boltz(V,85.*self.mis[10][0],10.*self.mis[10][1])
    #taumH(V::Float64) = tauX(V,272.,-1499.,42.2,-8.73)
    
    def tau_mH(self,V):
        return 50*tauX(V,6.*self.mis_t[9][0],5.5*self.mis_t[9][1],30*self.mis_t[9][2],-20*self.mis_t[9][3])
    
    # L-type calcium using mismatch previously used for S-type.
    def mL_inf (self,V):
        return boltz(V,45.*self.mis[5][0],-5.*self.mis[5][1])
    
    def tau_mL(self,V):
        return tauX(V,6.*self.mis_t[5][0],5.5*self.mis_t[5][1],30*self.mis_t[5][2],-20*self.mis_t[5][3])
    
    def mKCa_inf(self,Ca):
        return boltz(Ca,-30.*self.mis[8][0],-10.*self.mis[8][1])

    def tau_Ca(self):
        return 500*self.mis_t[7][0]
...
class neuron:
    ...
    def __init__(self,p:List[float],dyns,dyns2,mismatch=False,mismatch_dyn=False,mismatch_act=False,ob_type="V"):
        ...
        self.tauKCa=1
        self.taumCa=1
        self.tauCa = 500. # Without mismatch.
        self.taus=1
        self.alphaCa = 0.1
        self.betaCa = 0.04
        ...
        self.gL=p[5]  # Inward-rectifier potassium current maximal conductance
        ...
        self.theta=np.array([self.gNa,self.gH,self.gT,self.gA,self.gKd,self.gLeak,self.gKCa,self.gL])
    ...
    def sys_equ(self,t,u,V_pre:np.ndarray,V_pre_E:np.ndarray,input_noise=0):
        ...
        mKCa=u[9] # Not used, static fn
        mL=u[10] # L-type calcium current activation
        hS=u[11] # Not used
        Ca=u[12]
...
#                     -gleak*(V-Vleak) + Iapp)
        ...
        du1=1/self.C*(- self.gNa*mNa*hNa*(V-self.VNa) - self.gH*mH*(V-self.VH) - self.gT*mt*ht*(V-self.VCa) 
                 # - self.gA*mA**3*hA*(V-self.VK) 
                 - self.gKd*mKd*(V-self.VK)
                 - self.gLeak*(V-self.Vleak)
                 - self.gKCa*self.dyns.mKCa_inf(Ca)*(V-self.VK)
                 - self.gL*mL*(V-self.VCa)
                 +syn_i
        ...
        du5=1/max_abs(self.dyns.tau_mt(V),self.min_num)*(-mt+self.dyns.mt_inf(V))
        ...
        # du10=1/max_abs(self.dyns.tau_mKCa(),self.min_num)*(self.dyns.mKCa_inf(Ca) - mKCa)
        du10=0
        ...
        du11=1/max_abs(self.dyns.tau_mL(V),self.min_num)*(self.dyns.mL_inf(V) - mL)
        du12=0
        ...
        du13=(1/self.tauCa) * ((-self.alphaCa*self.gL*mL*(V-self.VCa))+(-self.betaCa*self.gT*mt*ht*(V-self.VCa)) - Ca)
