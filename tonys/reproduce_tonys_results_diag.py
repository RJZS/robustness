...
mis_t=(np.random.rand(12,4)*0.04-0.02)+1.0
...
# Noise Generation
noise=(np.random.normal(size=400000))*10 # sf for bursting
noise[0]=0
for i in range(len(noise)-1):
    noise[i+1]=noise[i]+(noise[i+1]-noise[i])/1000

noise=(noise-noise.mean())*2
plt.plot(noise[0:20000])
print(noise.var())
np.savez('noise_diag_model.npz',noise=noise,mis=mis3,mis_t=mis_t)

# loaded_data = np.load('noise_diag_model.npz')
# noise = loaded_data['noise']
# mis3 = loaded_data['mis']
# mis_t = loaded_data['mis_t']
